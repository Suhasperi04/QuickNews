...
def create_caption(news_items):
    """Create a caption that fits Instagram's limits (2200 chars)"""
    caption = "🌐 Additional Details:\n\n"
    
    # Calculate available space per news item
    total_items = len(news_items)
    # Reserve 150 chars for intro and hashtags
    available_space = 2050 / total_items if total_items > 0 else 2050
    
    # Add only the number and additional details for each news item
    for i, news in enumerate(news_items, 1):
        # Skip repeating the headline and jump straight to additional info
        desc = news.get('description', '').strip()
        
        # Calculate space for details
        detail_space = int(available_space)
        
        # Start with just the number
        caption += f"{i}. "
        
        # Add the description or additional details
        if desc:
            # If description is too long, truncate it
            if len(desc) > detail_space:
                desc = desc[:detail_space-3] + "..."
            caption += f"{desc}\n\n"
        else:
            # If no description, add a brief placeholder
            caption += "More details on this story as it develops.\n\n"
    
    # Add prompt to read caption
    caption += "📱 Read caption for more info!\n\n"
    
    # Add hashtags (limit to most relevant ones)
    caption += "#todaysnews #headlines #trending"
